...
class Travelagency
{
 public:
	void main_manu ()
	{
		cout << "\n\tBooking Office Name   :     HANZAI Travel Agency" << endl << endl;
		cout << "\tCONTACT NUMBER        :     0315-1537106" << endl  << endl;
		cout << "\tADDRESS               :     RAWALPINDI" << endl << endl;
...
public:
	void input()
	{
		...
		cin.getline(visachoice, sizeof(visachoice));
		system ("cls");
		cout << "\n\t**************************************************";
		cout << "\n\t\t\tTICKET CHOICES" << endl;
		cout << "\t**************************************************" << endl << endl;
        ...
        cin.getline(dplace, sizeof(dplace));
        cout << endl;
        cout << "\tEnter the Destination Place : ";
        cin.getline(lplace, sizeof(lplace));
        cout << endl;
}
	...
	void show ()
	{
		...
		cout << "\n\tDestination Place            :         " << lplace << endl << endl;	
	}
...
int main () 
{
	...
	while (1)
	{
		...
		case '1':
			...
			cout << "\t**************************************************" << endl;
			C.open ("customer.txt" , ios::out | ios::app);
			cust.input ();
			...
			cout << "\t**************************************************" << endl;
			R.open ("registration.txt" , ios::out | ios::app);
			reg.input ();
		...
		case '2':
			...
			cout << "\n\t\tShowing Data" << endl;
			cout << "\t**************************************************" << endl;
			C.open ("customer.txt" , ios::in);
			R.open ("registration.txt" , ios::in);
		...
		case '4':
			...
			switch (choice)
			{
				case '1':
					...
					mehroz = 0;
					C.open ("customer.txt" , ios::in|ios::out|ios::binary|ios::ate);
					C.seekg (0);
					while (C.read ((char*)&cust , sizeof (cust)))
					{
						if (match == cust.get_cust_id ())
				...
				case '2':
					...
					cin >> match;
					cin.ignore ();
					R.open ("registration.txt" , ios::in | ios::out | ios::binary | ios::ate);
					R.seekg (0);
					while (R.read ((char*)&reg , sizeof (reg)))
					{
						if (match == reg.get_reg_id())
		...
		case '5':
			...
			case '1':
				...
				cin >> match;
				C.open ("customer.txt" , ios::in|ios::binary);
				C1.open ("temp_customer.txt" , ios::out | ios::binary );
				while (C.read ((char*)&cust , sizeof (cust)))
				{
					if (match != cust.get_cust_id ())
					{
			...
			case '2':
				...
				cin >> match;
				R.open ("customer.txt" , ios::in | ios::binary);
				R1.open ("temp_registration.txt" , ios::out | ios::binary);
				while (R.read ((char*)&reg, sizeof (reg)))
				{
					if (match != reg.get_reg_id())
					{
				...
				rename ("temp_registration.txt" , "registration.txt");
				system("cls");
				cout << "\n\n\tData Deleted Successfully";
				Sleep(2000);
				goto flag2;
